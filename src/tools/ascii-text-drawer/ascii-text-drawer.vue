...
<template>
  <c-card style="max-width: 600px;">
    <c-input-text
      v-model:value="input"
      :label="t('tools.ascii-text-drawer.youtext')"
      :placeholder="t('tools.ascii-text-drawer.youdraw')"
      raw-text
      multiline
      rows="4"
    />
    ...
    <n-grid cols="4" x-gap="12" w-full>
      ...
      <n-gi span="2">
        <n-form-item :label="t('tools.ascii-text-drawer.width')" label-placement="top" label-width="100" :show-feedback="false">
          <n-input-number v-model:value="width" min="0" max="10000" w-full :placeholder="t('tools.ascii-text-drawer.widthtext')" />
        </n-form-item>
      </n-gi>
    </n-grid>
    ...
    <div v-if="processing" flex items-center justify-center>
      <n-spin size="medium" />
      <span class="ml-2">{{ $t('tools.ascii-text-drawer.loadfont') }}</span>
    </div>
    ...
    <n-form-item v-if="!processing && !errored" :label="t('tools.ascii-text-drawer.airtext')">
      <TextareaCopyable
        :value="output"
        mb-1 mt-1
