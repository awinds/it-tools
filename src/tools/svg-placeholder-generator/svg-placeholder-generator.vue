...
const { copy: copySVG } = useCopy({ source: svgString, text: '复制成功' });
const { copy: copyBase64 } = useCopy({ source: base64, text: '复制成功' });
...
<template>
  <div>
    <n-form label-placement="left" label-width="100">
      ...
      <div flex gap-3>
        <n-form-item label="高度 (in px)" flex-1>
      ...
      <div flex gap-3>
        ...
        <c-input-text
          ...
          label="自定义文本"
          :placeholder="`默认是 ${width}x${height}`"
          label-position="left"
          label-width="100px"
          label-align="right"
          flex-1
