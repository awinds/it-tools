...
const { attrs: bitsValidationAttrs } = useValidation({
  ...
  rules: [
    {
      message: t('tools.rsa-key-pair-generator.bits-error'),
...
<template>
  <div style="flex: 0 0 100%">
    <div item-style="flex: 1 1 0" style="max-width: 600px" mx-auto flex gap-3>
      <n-form-item :label="t('common.noun.bits')" v-bind="bitsValidationAttrs as any" label-placement="left" label-width="100">
        <n-input-number v-model:value="bits" min="256" max="16384" step="8" />
      </n-form-item>

      <c-button @click="refreshCerts">
        {{ $t('tools.rsa-key-pair-generator.refresh-key') }}
      </c-button>
    </div>
  </div>
