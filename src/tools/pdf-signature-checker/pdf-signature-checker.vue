...
<template>
  <div style="flex: 0 0 100%">
    <div mx-auto max-w-600px>
      <c-file-upload :title="t('tools.pdf-signature-checker.fileupload')" accept=".pdf" @file-upload="onVerifyClicked" />
  ...
  <div v-if="status === 'parsed' && signatures.length" style="flex: 0 0 100%" mt-5 flex flex-col gap-4>
    <div v-for="(signature, index) of signatures" :key="index">
      <div mb-2 font-bold>
        {{ t('tools.pdf-signature-checker.Signature') }} {{ index + 1 }} {{ t('tools.pdf-signature-checker.certificates') }} :
      </div>
