...
const { t } = useI18n();
const { copy } = useCopy({ source: header, text: t('common.clipboard.success') });
</script>

<template>
  <div>
    <c-input-text v-model:value="username" :label="t('tools.basic-auth-generator.username')" :placeholder="t('tools.basic-auth-generator.username')" clearable raw-text mb-5 />
    <c-input-text
      v-model:value="password"
      :label="t('tools.basic-auth-generator.password')"
      :placeholder="t('tools.basic-auth-generator.password')"
      clearable
      raw-text
      mb-2
      type="password"
