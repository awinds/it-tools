...
<template>
  <div>
    <c-card v-if="!isSupported">
      {{ $t('tools.camera-recorder.not-support-camera') }}
    </c-card>

    <c-card v-else-if="!permissionGranted" text-center>
      {{ $t('tools.camera-recorder.grant-permission-message') }}

      <c-alert v-if="permissionCannotBePrompted" mt-4 text-left>
        {{ $t('tools.camera-recorder.blocked-message') }}
      </c-alert>

      <div v-else mt-4 flex justify-center>
        <c-button @click="requestPermissions">
          {{ $t('tools.camera-recorder.grant-permission') }}
        </c-button>
      </div>
    </c-card>
      ...
      <div v-else>
        ...
        <div flex items-center justify-between gap-2>
          <c-button :disabled="!isMediaStreamAvailable" @click="takeScreenshot">
            ...
            {{ $t('tools.camera-recorder.takescreen') }}
          </c-button>
          ...
          <div v-else italic op-60>
            {{ $t('tools.camera-recorder.not-support-video') }}
          </div>
        </div>
      </div>
    </c-card>
